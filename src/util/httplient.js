...
export class Httpclient {
  ...
  get(url,params,fun) {
    if(this.isopen=="isopen"?false:true&&new auth().getToken()==null){
      Message.error('帐号身份己过期,请重新登录')
     if(new auth().getIsCaslogin()){
        //如果是cas的需要跳转的cas登录页
        window.parent.location.href = sessionStorage.getItem("loginUrl");
      }else{
       router.push({"path":"/login"});
      }
    }else{
  ...
  notokenget(url,params,fun) {
    console.log('路径：',this.httpApicode+url);
    instance1.get(this.httpApicode+url, {
      params: params,
