var TreeHelper = function () {
  if (this instanceof TreeHelper) {
    ...
    this.currenType = "tree"; //当前类型：tree-树，menuTree-菜单树
    this.limitZtreeHeightOfValue = 100; //树的差值，在内部树去掉边框边距内间距等
    this.setLimitZtreeHeight = function(value,type){ //限制组件的高度
      if(value){
        that.limitZtreeHeight = value;
      }
      if(type&&type=="menuTree"){
        // 菜单树去掉的高度，按钮的高度和边距的高度
        that.limitZtreeHeightOfValue = 36;
      }
    };
    this.setting = {
      view: {
    ...
    this.ychfun = null;
    this.curMenu = null;
    this.menuType = null;//simple default diy
    this.curMenuType = 1;
    ...
    this.initView = function () {
      ...
      if (that.limitZtreeHeight != null) {
        that.limitZtreeHeight>100?that.limitZtreeHeight=that.limitZtreeHeight-that.limitZtreeHeightOfValue:that.limitZtreeHeight;
    ...
    //创建简单菜单，有加号图标
    this.createSimpleOfIconMenu = function (viewId, zNodes, zcfun) {
      this.menuType = "simpleOfIccon";
      this.template = "ztreeOfIcon_section";
      this.viewId = viewId;
      this.sysMenuBtn = "createSimpleOfIcconMenuBtn";      
      this.formatSimpleData(zNodes);
      this.zNodes = zNodes;
      this.zcfun = zcfun;
      this.init();
    ...
    // 创建菜单，有加号图标,可操作右键菜单
    this.createSimpleOfIconOperateMenu = function (viewId, zNodes, zcfun, ycqfun, ychfun) {
      this.template = "ztreeOfIcon_section";
    ...
    this.onBodyMouseDown = function (event) {
      if (!(event.target.id == "rMenu" || $(event.target).parents(that.rMenuKey).length > 0)) {
        ...
        $(that.rMenuKey).css({"display": "none"});
      }
    };
